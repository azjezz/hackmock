...
use type Usox\HackMock\Exception\MissingMethodCallException;
use type Usox\HackMock\Exception\UnexpectedMethodCallException;
use function Facebook\FBExpect\expect;
use function Usox\HackMock\mock;
use function Usox\HackMock\prospect;
use function Usox\HackMock\tear_down;

class SampleTest extends HackTest\HackTest {
  ...
  public function testNoParamsAndVoidButThrows(): void {
    ...
    expect(() ==> $sample->noParamsAndVoidButThrows())
  ...
  public function testNoParamsButReturnsInt(): void {
    ...
    expect($sample->noParamsButReturnsInt())
  ...
  public function testNoParamsButReturnsString(): void {
    ...
    expect($sample->noParamsButReturnsString())
      ->toBeSame($return_value);
  }
  ...
  public function testNoParamsButReturnsSampleInterfaceInstance(): void {
    ...
    expect($sample->noParamsButReturnsSampleInterfaceInstance())
  ...
  public function testParamsValidationSucceeds(): void {
    ...
    expect($sample->basicParamValidation($int, $string, $float, $class))
      ->toBeNull();
  }
  ...
  public function testParamsValidationFailesWithDifferentCallCountsButOneWrongParameter(
  ): void {
    $sample = mock(Stub\SampleInterface::class);
    $int = 1234;
    $string = 'string';
  ...
  public function testParameterValidationWithClosure(): void {
    ...
    expect($sample->paramValidationWithClosure($string))
      ->toBeNull();
  }
  ...
  public function testParamsValidationFails(): void {
    ...
    expect(
      ...
      ->toThrow(
        UnexpectedMethodCallException::class,
        Str\format(
          'No expectation defined for `%s::basicParamValidation` with parameter `5678,nostring,4.2,stdClass`',
          \get_class($sample),
        ),
      );
  }
  ...
  public function testHavingMoreCallsThenExpectationsThrowsException(): void {
    ...
    expect(
      ...
      ->toThrow(Usox\HackMock\Exception\UnexpectedMethodCallException::class);
  }
  ...
  public function testIntParamAndIntReturnValueOnBaseClass(): void {
    ...
    expect($sample->intParamAndReturnsInt($int1))
      ->toBeSame($int2);
  }
